...
# ISSUES: We need to first stop splunk in all the machines we are intending to use this script(Almost done because we may need to also stop splunk when running as a service, or use 'sudo', but that is disrty)
# We need to type the password of the remote user everytime there is a command in the script that requires a user actions(we may avoid this if we use private keys ssh)
# We need to run the chown command to change the ownership of the tar unzipping on the '/opt' directory, "sudo chown splunk:splunk -R splunkforwarder"
# We need to run splunk after doing the 'chown' command, because this does not run splunk after doing that
...
HOSTS_FILE="/home/kel/splunk-script-hosts-file.txt"
...
SPLUNK_FILE="/home/kel/splunkforwarder-9.4.3-237ebbd22314-linux-amd64.tgz"
...
# This is the directory where the splunk tar installation will be untared, for example 'splunkforwarder' or 'splunk' for example
# Edit the directory in case you are installing one or the other
...
REMOTE_SCRIPT='
  fail() {
    ...
    echo ">>> Checking for existing Splunk installations..."

    # Define possible install paths
    POSSIBLE_SPLUNK_DIRS=("'"$NEW_PARENT"'/splunk" "'"$NEW_PARENT"'/splunkforwarder")

    for SPLUNK_DIR in $POSSIBLE_SPLUNK_DIRS; do
    if [ -x "$SPLUNK_DIR/bin/splunk" ]; then
        echo ">>> Stopping existing Splunk instance at $SPLUNK_DIR"
        "$SPLUNK_DIR/bin/splunk" stop || echo "Stop failed or not running"

        BACKUP_DIR="${SPLUNK_DIR}_backup_$(date +%Y%m%d%H%M%S)"
        echo ">>> Backing up $SPLUNK_DIR to $BACKUP_DIR"
        mv "$SPLUNK_DIR" "$BACKUP_DIR" || fail "Failed to backup $SPLUNK_DIR"
    fi
    done

    echo ">>> Extracting new Splunk UF tarball..."
(cd "'"$NEW_PARENT"'" && tar -zxf "'"$DEST_FILE"'") || fail "Could not untar Splunk package"
'

# echo ">>> Stopping existing Splunk UF (if running)..."
# if [ -x "'"$NEW_INSTANCE"'/bin/splunk" ]; then
#   "'"$NEW_INSTANCE"'/bin/splunk" stop || echo "No active instance or stop failed"
#   echo ">>> Backing up existing UF..."
#   mv "'"$NEW_INSTANCE"'" "'"${NEW_INSTANCE}_backup_$(date +%Y%m%d%H%M%S)"'" || fail "Failed to backup UF"
# fi
