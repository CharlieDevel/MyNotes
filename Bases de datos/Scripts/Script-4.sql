...
drop procedure ReassignTreesPositionsIfTooCloseToEachOther;
	...
	BEGIN
	   ...
	   --Get the values for the current tree
		...
		BEGIN
			...
			BEGIN
				UPDATE Tree 
				SET PositionX = PositionX + (RAND() * @maxrangeforX_reassign) , PositionZ = PositionZ + (RAND() * @maxrangeforZ_reassign)
				WHERE TreeId = @index;
...
exec ReassignTreesPositionsIfTooCloseToEachOther @maxrangeforX_reassign = 10.9, @maxrangeforZ_reassign = 7.8;
